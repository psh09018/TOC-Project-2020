...
machine = TocMachine(
    ...
    transitions=[
        # start-schedule
        { "trigger": "advance", "source": "start", "dest": "schedule", "conditions": "is_going_to_schedule",},
        # start-team_info
        { "trigger": "advance", "source": "start", "dest": "team_info", "conditions": "is_going_to_team_info",},
        # start-detail
        { "trigger": "advance", "source": "start", "dest": "detail", "conditions": "is_going_to_detail",},
        # start-fsm
        { "trigger": "advance", "source": "start", "dest": "fsm", "conditions": "is_going_to_fsm",},
        # start-meme
    ...
    for event in events:
        ...
        if response == False:
            if machine.state != 'start' and event.message.text.lower() == 'restart':
                ...
                machine.go_back()
            elif machine.state == 'start' and event.message.text.lower() == 'restart':
                information = "歡迎使用女排聯賽機器人\n\
                               請輸入『賽程』、『隊伍資訊』、『活動詳情』獲得比賽相關資訊\n\
                               也可輸入『meme』讓你獲得快樂\n\
                               隨時輸入『restart』重新開始\n\
